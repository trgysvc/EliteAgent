...
public actor MemoryAgent: AgentProtocol {
    ...
    public func receive(_ signal: Signal) async throws {
        if signal.name == "THINK_BLOCK" {
            // v13.8: UNO Pure - Binary PropertyList Decoding (No JSON Artıkları)
            if let block = try? PropertyListDecoder().decode(MemoryThinkBlock.self, from: signal.payload) {
    ...
    private func archiveTaskHistoryIfNeeded() {
        let paths = PathConfiguration.shared
        let historyURL = paths.applicationSupportURL.appendingPathComponent("task_history.plist")
        ...
        if Date().timeIntervalSince(modDate) > 30 * 24 * 3600 {
            let archiveURL = paths.applicationSupportURL.appendingPathComponent("task_history_archive.plist")
