...
public struct MediaControllerTool: AgentTool {
    ...
    public let ubid: Int128 = 43 // Token 'L' in Qwen 2.5
    ...
    public func execute(params: [String: AnyCodable], session: Session) async throws(AgentToolError) -> String {
        ...
        do {
            ...
            case "volume":
                let finalLevel: Int
                if let level = params["level"]?.value as? Int {
                    finalLevel = level
                } else {
                    let fallbackLevel = params["level"]?.value as? Double
                    finalLevel = Int(fallbackLevel ?? 50)
                }
            ...
            if let toolError = error as? AgentToolError { throw toolError }
